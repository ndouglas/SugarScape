...
- **Licensing (binding).** `data/anasazi/` holds `Map.txt`, `adjustedPDSI.txt`, `environment.txt`, `water.txt` and `settlements.txt` **unmodified** (their SHA-256 sums are in Task 1), the package's GPL-2.0 `LICENSE` and `CITATION.cff`, a `NOTICE` (GPL-2.0, doi:10.25937/krp4-g724, separate from the MIT code) and the derived `historical.txt`. The model code is written from the ODD and JASSS as captured in the extraction: **never read, translate or paraphrase `lhv.nlogo`'s procedures**; where the extraction says "the replication does X", the code implements that behaviour from the extraction's words (Decisions 3–9 spell each one out).
...
- **Honesty.** Presets, sweep descriptions, the README and the Rules panel say this reproduces Janssen's 2009 replication (itself an approximation of Axtell et al. 2002, A-20), which behaviours come from the replication rather than the written description, and how close each preset gets to the historical record as measured (Decision 21).
...
8. **The ten quirks, exactly** (`quirks` in the config; each `bool`, default `true`; `Quirks::ALL` in the published presets, `Quirks::NONE` in `lhv-documented`): `age_before_death_check` (A-2: age at step 1, before the death check; no ageing at step 9) · `no_farm_water_check` (A-3: no water radius for farms) · `uplands_single_class` (A-6: Arable Uplands' PDSI is 0, so always 547 kg, instead of the Natural series) · `wrap_edges` (A-9: every distance wraps, 80 × 120 torus) · `initial_corn_per_slot` (A-11: an initial-corn draw in each of the three slots; off: one draw, in `S0`) · `fission_fresh_endowment` (A-12, Decision 7) · `fission_needs_free_farm` (A-12, Decision 7) · `initial_eligibility_ignores_adjustment` (A-13: initial farms by `y · q`) · `occupancy_leak` (A-19: every settling — initial, move, fission — counts **two** in `settled`, a mover never frees its old home, each removal frees one, so a cell once lived on stays closed to farming; off: counts are exact) · `single_harvest_variance` (A-1: soil uses the annual s.d.). The Dunes' constant 855 kg, Kinbiko on the North series, hydro, water and 93.5 m are not quirks (adopted, Decisions 1, 5, 6).
...
data/anasazi/NOTICE, historical.txt                 NEW  licence notice; the derived household curve (1)
...
### Task 1: The valley's data, its licence, and its parser
...
- [ ] **Step 1: Copy the five data files, the licence and the citation, unmodified, and the historical curve**
...
MIT-licensed (see the LICENSE file at the repository root); the MIT licence
...
*Needs judgement only if a measurement differs from Decision 21 (then stop and report; do not retune). Otherwise mechanical (full code).* Browser (controller): nothing to check (Experiments lists the two sweeps once Task 11's page work is in; the controller runs them there in Task 12).
...
Run: `wasm-pack test --node crates/sugarscape-wasm`
Expected: PASS (29 tests). `an_anasazi_sim_matches_the_native_golden_entry_and_finishes` is the portability check of Decision 4: the wasm32 fingerprint of `lhv-published` after 200 ticks is the native `0x3b357e6f0cc5f74a`. If it differs, stop and report — some rule is drawing a `usize` range or calling a platform maths function.
...
*Mechanical (full code).* Browser (controller): on `lhv-published` the Charts tab shows **Households vs historical** (Simulated and the grey Historical estimate, which falls to 0 in 1300), **Carrying capacity**, **Fit**, **Mean stored corn** and **Births, moves and departures**, with a **Year** axis from 800; none of the sugarscape's charts or sections; in the Compare entry B's lines are dashed and the historical line is drawn once; Export → Charts writes the five PNGs.
...
- *Documented rules* — Task 3 `world.rs` (Decisions 3, 5, 7), `household.rs` (Task 2): harvest, storage ageing and consumption, removal, expected harvest, farm plot, the settlement fallback chain, fission and endowment, water update, ageing last.
