...
4. Growback, pollution diffusion, replacement, aging (as in milestone 1, replacement before aging).
5. Stats.
...
- **Deaths:** borrower dies → loan canceled (lender's loss). Lender dies → loan canceled, unless inheritance (I) is on, in which case the claim is split equally among the lender's living children (one new loan per child, same terms and due tick; a child who is itself the borrower has its share forgiven, so no one owes itself).
