...
| `outbreaks: Vec<Outbreak { tick: u64, agents: u32 }>` | at `tick`, a brand-new random disease infects `agents` random living agents | empty |

**Validation:** `immune_length` 1–64; `length.min ≥ 1`; `length.max < immune_length`; `initial ≤ count`; `count` 1–1000; `fee` finite ≥ 0; `flips_per_tick ≥ 1`; mutation probabilities in [0, 1]; each outbreak `tick ≥ 1`, `agents ≥ 1`.
...
| id | Rules | Parameters |
|---|---|---|
| `v-1-rid` | ({G₁}, {M, E}) | Chapter II agents; disease on: 10 diseases, lengths 1–10, 4 per agent, immune length 50 (Animation V-1: society rids itself of disease) |
| `v-2-endemic` | ({G₁}, {M, E}) | as `v-1-rid` with 25 diseases, 10 per agent (Animation V-2: endemic disease) |
| `v-mcneill` | ({G₁}, {M, S, E}) + outbreak | `v-1-rid` disease setup with Chapter III demography; outbreak at t = 300 infecting 5 agents |
...
- **Book reproductions (`#[ignore]`, release, measured values in comments):** `v-1-rid` reaches zero infected; `v-2-endemic` still has infected agents at t = 1000; `v-mcneill` infections rise after the t = 300 outbreak.
