...
- The toolbar gains **⟲1** (back one tick) and a **timeline slider** from 0 to `reached`, the furthest tick on this world's current branch. Dragging the slider or pressing ⟲1 moves the world to that exact tick; the grid, charts and Inspect show what they showed then; a followed agent stays followed (its trail is the keyframe's if it was followed then, else it starts at the seek).
