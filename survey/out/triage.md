...
Causes: **setup** (the preset's configuration can't produce what it describes), **description** (the model
follows its rules; the text misstates or overstates), **book** (the rules match but the book's result doesn't
appear), **check** (the claim's check or threshold doesn't test what its source says; the model is fine),
**power** (right direction, too few seeds). No verdict was traced to a **model bug**.
...
| iv-1.shuttle | Fails | description | 49% (median) of agents switch between sugar and spice sites twice in t=100..200, against a ≥ 50% threshold: about half shuttle. "Agents shuttle" is true of about half of them. |
...
| iv-18.falls | Weak | book | Mean foresight 4.98 → 4.41 by t=1000, p=0.13; not reliably selected down. The repo's book test passes on its few seeds only. |
| iv-18.modest-kept | Weak | description | At t=2000 foresight is in [0.1, 5] on 12/20 seeds; 8/20 stay above the initial mean. "Evolution keeps a modest foresight" overstates a drift that often doesn't happen. |
...
| vi-3.book-trade-raises | Weak | book | Trade vs no-trade plateau 826 vs 809, p=0.16; equivalence holds (vi-2.like-vi-3). Known. |
...
| fig-iv-6.trade-raises | Weak | power | At the file's 10 seeds, p < 0.01 at visions 1, 3, 4 only. Independent 20-seed rerun (CLI + scipy): p = 8e-5, 2e-4, 3e-5, 1e-3, 8e-3, 1.3e-2 for visions 1–6. The gain narrows with vision (+8 → +5). |
| fig-iv-6.measured-no-trade-v1 | Weak | check | Sweep means reproduce exactly (33.85, recorded 33.8); the claim judged each seed against ±10% of the mean. |
| fig-iv-10-11.measured-long-last | Weak | check | Same per-seed-vs-mean mismatch; the recorded mean reproduces. |
| n-goods-carrying-capacity.measured-no-trade-2 | Fails | check | Mean reproduces exactly (25.84, recorded 25.8). |
| bargaining-rules.measured-geometric-v1 | Fails | check | Same runs as fig-iv-6's trade line; mean reproduces exactly (41.78, recorded ~41.8). |
| bargaining-rules.measured-random-v6 | Weak | check | Same per-seed-vs-mean mismatch. |
