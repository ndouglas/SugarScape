 ...
 */
export function buildExportMenu(opts: ExportOptions): HTMLDetailsElement {
  const items = h('div', { class: 'menu-items' });
  const menu = h('details', { class: 'menu' }, h('summary', {}, 'Export'), items);
  const guard = (run: () => Promise<void>): void => {
    run().catch((e) => showNotice(`Could not export (${errorMessage(e)})`, 10_000));
  };
  const perWorld = (name: string, run: (world: ExportWorld) => Promise<void>): HTMLElement => {
    const worlds = opts.worlds();
    if (worlds.length === 1) return h('button', { onclick: () => guard(() => run(worlds[0])) }, name);
    return h(
      'div',
      { class: 'menu-row' },
      h('span', {}, name),
      ...worlds.map((w) => h('button', { title: `${name} of world ${w.label}`, onclick: () => guard(() => run(w)) }, w.label)),
    );
  };
  const fill = (): void => {
    items.replaceChildren(
      ...
      h(
        'button',
        {
          ...
          onclick: () => opts.session().catch((e) => showNotice(`Could not build the session file (${errorMessage(e)})`, 10_000)),
        },
        'Session (JSON)',
      ),
    );
